...
{
    if (v1 == v2 && x1 != x2) { // If the kangaroos have the same speed, they will never meet
        return "NO";
    } 
    else 
    {
        double n = static_cast<double>(x2 - x1) / (v1 - v2);
        if (n >= 0 && n == static_cast<int>(n)) { // If n is a non-negative integer, the kangaroos will meet
            return "YES";
        } 
        else 
        {
